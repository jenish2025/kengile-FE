...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'About', href: '/about-us' },
    {
      ...
      href: '/services',
      submenu: [
        { name: 'AI Infrastructure', href: '/services/ai-infrastructure-services', icon: '🤖' },
        { name: 'Data Center Modernization', href: '/services/data-center-modernization', icon: '🏢' },
        { name: 'Cloud & HPC Engineering', href: '/services/cloud-hpc-engineering', icon: '☁️' },
        { name: 'DevOps & SRE', href: '/services/devops-sre-automation', icon: '⚙️' },
        { name: 'Data Engineering', href: '/services/data-engineering-analytics', icon: '📊' },
        { name: 'AI Application Development', href: '/services/ai-application-development', icon: '🧠' },
        { name: 'Smart Energy Solutions', href: '/services/smart-energy-it-solutions', icon: '⚡' },
        { name: 'Green Cloud', href: '/services/green-cloud-sustainable-it', icon: '🌱' },
        { name: 'Blockchain Infrastructure', href: '/services/blockchain-infrastructure-services', icon: '🔗' },
        { name: 'Space Connectivity', href: '/services/satellite-space-internet-solutions', icon: '🛰️' },
      ],
    },
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled 
          ? 'bg-white/80 backdrop-blur-2xl shadow-xl border-b border-primary-100/50' 
    ...
    >
      ...
      <div className={`absolute inset-0 bg-gradient-to-b from-white/10 to-transparent pointer-events-none transition-opacity duration-500 ${
        ...
        <div className="flex justify-between items-center h-20">
          <Link href="/" className="flex items-center space-x-3 group">
            ...
            >
              ...
              <div className={`relative text-2xl md:text-3xl font-black transition-all duration-300 text-gradient`}>
                Kengile
              </div>
            </motion.div>
            <motion.div
              ...
              <div key={link.name} className="relative group">
                {link.submenu ? (
                  ...
                  >
                    <Link 
                      href={link.href} 
                      className="text-accent-700 hover:text-primary-600 font-semibold transition-colors"
                    >
                      {link.name}
                    </Link>
                    <ChevronDown 
                      className={`w-4 h-4 text-accent-600 transition-transform duration-300 ${
                  ...
                  <Link
                    href={link.href}
                    className="relative px-4 py-2 text-accent-700 hover:text-primary-600 font-semibold transition-colors rounded-lg hover:bg-primary-50/50 block"
                  >
                    {link.name}
          ...
          <motion.button
            ...
            className="lg:hidden p-2 text-accent-700 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-all"
          >
            <motion.div
